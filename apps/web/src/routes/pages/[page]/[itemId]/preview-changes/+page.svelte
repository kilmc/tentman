	...
	const documentTitle = $derived(
		formatAppTitle(
			buildContentTitleContext({
				...
				item:
					data.contentData && typeof data.contentData === 'object'
						? (data.contentData as ContentRecord)
						: null
			}),
			siteName
		)
...
<div class="mx-auto max-w-4xl">
	<div class="mb-6">
		...
		<p class="mt-2 text-sm text-stone-500">
			This screen only covers this item. Use <strong>Review Draft</strong> to inspect the
			current draft across the whole site before publishing.
		</p>
		{#if data.isNew}
			<p class="mt-1 text-sm text-stone-500">Creating new item</p>
		{:else}
			<p class="mt-1 text-sm text-stone-500">Editing: {data.itemId}</p>
		{/if}
	</div>
	...
	{:else if displayedChangesSummary}
		...
		<div class="mt-5 rounded-md border border-stone-200 bg-stone-100 p-4">
			...
			<ul class="list-inside list-disc space-y-1 text-sm text-stone-700">
				...
				<li>
					<strong>Publish Draft:</strong> Saves to the managed Tentman draft, publishes it immediately, and returns you to the workspace.
				</li>
			</ul>
		</div>
	{/if}
