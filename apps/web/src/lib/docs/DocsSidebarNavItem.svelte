	...
	function getBranchClasses(open: boolean, level: number): string {
		...
		const depthClass =
			level <= 0
				? 'font-medium text-stone-700 hover:bg-white hover:text-stone-950'
				: 'text-stone-600 hover:bg-white hover:text-stone-950';
		const openClass = open ? 'bg-white text-stone-950 shadow-[inset_0_0_0_1px_rgba(28,25,23,0.08)]' : '';
...
{#if hasChildren}
	<Collapsible.Root open={isOpen} class="group">
		<Collapsible.Trigger
			class={getBranchClasses(isOpen, depth)}
		>
		...
		<Collapsible.Content class="overflow-hidden">
			...
			>
				{#if item.href}
					<a
						href={item.href}
						class={getLeafClasses(isActive, depth + 1)}
					>
						Overview
					</a>
				{/if}
...
{:else if item.href}
	<a
		href={item.href}
		class={getLeafClasses(isActive, depth)}
	>
		{item.title}
	</a>
{/if}
