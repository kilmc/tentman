...
- Returning from `/publish` to `/pages` by clicking the site title took about 2.5s in this live run. The trace showed a fresh pages/config path: `/api/repo/configs` took 657.3ms, instruction discovery probed `tentman/instructions` and got a 404, pages summary took 0.2ms, config states took 533.8ms, and freshness took 599.4ms.
...
- Treat return from `/publish` to `/pages` as its own warm-return workflow or optimize it to reuse the already-known workspace bootstrap/overview data when identities have not changed. Tracked by issue 23.
