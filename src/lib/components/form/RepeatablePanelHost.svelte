<script lang="ts">
	...
	let draftItem = $state<ContentRecord>(cloneContentRecord(panel.selectedItem));
	let savedFingerprint = $state(getFingerprint(panel.selectedItem));
	let isDirty = $derived(getFingerprint(draftItem) !== savedFingerprint);

	$effect(() => {
		draftItem = cloneContentRecord(panel.selectedItem);
		savedFingerprint = getFingerprint(panel.selectedItem);
	});

	function cloneContentRecord(item: ContentRecord): ContentRecord {
		try {
	...
	function getFingerprint(item: ContentRecord): string {
	...
	function saveChanges() {
		...
		panel.save(cloneContentRecord(draftItem));
		savedFingerprint = getFingerprint(draftItem);
	}
...
>
	...
	>
		...
		{#if panel.mode === 'edit' && panel.remove}
			<button
				type="button"
				onclick={removeItem}
