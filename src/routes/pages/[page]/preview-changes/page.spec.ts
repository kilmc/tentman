...
describe('routes/pages/[page]/preview-changes/+page', () => {
	it('preserves preview query params when redirecting unauthenticated users to login', async () => {
		await expect(
			...
			status: 302,
			location:
				'/auth/login?redirect=%2Fpages%2Fabout%2Fpreview-changes%3Fdata%3Dabc%26branch%3Dpreview-2026-04-06'
		});
	});
	...
	it('returns the explicit branch so singleton preview can round-trip draft context', async () => {
		...
		await expect(
			load({
				...
				url: new URL('http://localhost/pages/about/preview-changes?data=abc&branch=preview-2026-04-06'),
				depends: () => {}
			} as never)
		).resolves.toMatchObject({
