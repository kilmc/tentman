...
<div class="container mx-auto max-w-5xl p-6">
	...
	<h1 class="mb-2 text-3xl font-bold">Publish Changes</h1>
	<p class="mb-8 text-gray-600">Review and publish all draft changes to the main branch.</p>

	<div class="mb-8 rounded-lg border border-blue-200 bg-blue-50 p-4">
		<p class="text-sm text-blue-800">
			<span class="font-medium">Draft Branch:</span>
			<code class="ml-2 rounded bg-blue-100 px-2 py-1 text-xs">{data.draftBranch.name}</code>
		</p>
	...
	<div class="mb-8">
		<h2 class="mb-4 text-xl font-semibold">Changed Content</h2>
		<div class="space-y-3">
			{#each data.configsWithChanges as { config, changes }}
				<div class="rounded-lg border border-gray-200 bg-white p-4">
					<div class="flex items-start justify-between">
						...
						<a href="/pages/{config.slug}" class="text-sm text-blue-600 hover:underline"> View </a>
					</div>
				</div>
			{/each}
		</div>
	...
	<!-- Commits -->
	<div class="mb-8">
		<h2 class="mb-4 text-xl font-semibold">Commits ({data.commits.length})</h2>
		<div class="space-y-2">
	...
	<div class="flex flex-col gap-3 border-t border-gray-200 pt-6 sm:flex-row">
		...
		>
			<button
				...
				class="w-full rounded-lg bg-green-600 px-6 py-3 font-medium text-white transition-colors hover:bg-green-700 disabled:cursor-not-allowed disabled:bg-gray-400 sm:w-auto"
			>
				{#if publishing}
					Publishing...
				{:else}
		...
		>
			<button
				type="submit"
				disabled={publishing || discarding}
				class="w-full rounded-lg border border-red-300 bg-white px-6 py-3 font-medium text-red-700 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-400 sm:w-auto"
			>
				{#if discarding}
					Discarding...
				{:else}
		...
		<a
			href="/pages"
			class="w-full rounded-lg border border-gray-300 bg-white px-6 py-3 text-center font-medium text-gray-700 transition-colors hover:bg-gray-50 sm:w-auto"
		>
			Cancel
		</a>
	</div>
