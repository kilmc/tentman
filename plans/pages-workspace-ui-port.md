...
The first implementation should include the full pages UI: left site sidebar, top page actions, collection index/editor layout, singleton form width cap, and structured repeatable-block side editor.
...
- Replace the current top-header-heavy pages experience with a pages workspace shell:
  ...
  - Topbar shows the current page title plus `Preview` and `Publish` actions using existing availability rules.
...
- Move collection item navigation into a dedicated collection index pane:
  - Collection index is visible beside item view/edit/new routes on desktop.
  - Selecting an item does not collapse the index.
  - Collapsed index mode uses an explicit hide/show control tied to the index rail.
  - `New {itemLabel}` lives in the collection index pane, not beside Publish.
- Add collection index sorting:
...
- If helper logic grows, add focused modules under `src/lib/features/content-management` for collection index view models and under `src/lib/features/forms` for repeatable item labels.
...
- Add or adjust browser component tests for:
  ...
  - Collection item routes render the collection index beside the editor and keep it visible after item selection.
  - Collection index sort switches between Custom and title sorting, and Custom edit mode updates the navigation manifest flow.
...
- Collection item ordering belongs in the collection index pane, not the global site sidebar.
